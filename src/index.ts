...
function parseCsvAsState(csvTimeEntries: string, csvAsana: string) {
  const timeEntries: TimeEntry[] = csvTimeEntries
    .split(/\r\n|\n/)
    .map((entry: string) => {
...
function getHoursPracticedPerMonthBarChart(
  ...
  const hoursPracticedPerMonth = Object.fromEntries(
    Object.entries(
      groupBy(timeEntries, (te) =>
        te.startDate.toLocaleString("en-us", groupFormatting)
      )
    ).map(([key, value]) => [
      key,
          ...
          callbacks: {
            ...
            afterLabel: (context) => {
              ...
              return asanaLearnedInMonth.length == 0
                ...
                : "Asana learned in that month: " +
                    asanaLearnedInMonth.join(", ");
            },
            label: (context) => {
